...
impl<T: Lattice + Clone> Lattice for Rc<T> {
    fn meet_mut(&mut self, other: Self) -> bool {
        match self.as_ref().partial_cmp(&other) {
            ...
            None => Rc::make_mut(self)
                .meet_mut(Rc::try_unwrap(other).unwrap_or_else(|rc| (*rc).clone())),
        }
    }
    ...
    fn join_mut(&mut self, other: Self) -> bool {
        match self.as_ref().partial_cmp(&other) {
            ...
            None => Rc::make_mut(self)
                .join_mut(Rc::try_unwrap(other).unwrap_or_else(|rc| (*rc).clone())),
        }
    }
}
...
impl<T: Lattice + Clone> Lattice for Arc<T> {
    fn meet_mut(&mut self, other: Self) -> bool {
        match self.as_ref().partial_cmp(&other) {
            ...
            None => Arc::make_mut(self)
                .meet_mut(Arc::try_unwrap(other).unwrap_or_else(|rc| (*rc).clone())),
        }
    }
    ...
    fn join_mut(&mut self, other: Self) -> bool {
        match self.as_ref().partial_cmp(&other) {
            ...
            None => Arc::make_mut(self)
                .join_mut(Arc::try_unwrap(other).unwrap_or_else(|rc| (*rc).clone())),
        }
    }
}
