...
use crate::internal::{CRelIndexWrite, Freezable, RelIndexMerge, RelIndexWrite};
use crate::rel_index_read::{CRelIndexRead, CRelIndexReadAll, RelIndexRead, RelIndexReadAll};
...
impl<K: Clone + Hash + Eq, V: Clone + Hash + Eq> CLatIndex<K, V> {
    #[inline]
    pub fn unwrap_frozen(
        &self,
    ) -> &dashmap::ReadOnlyView<K, SetType<V>, BuildHasherDefault<FxHasher>> {
        match self {
    ...
    #[inline]
    pub fn unwrap_mut_unfrozen(
        &mut self,
    ) -> &mut DashMap<K, SetType<V>, BuildHasherDefault<FxHasher>> {
        match self {
            CLatIndex::Unfrozen(dm) => dm,
            CLatIndex::Frozen(_) => panic!("CRelIndex::unwrap_unfrozen(): object is Frozen"),
        }
...
impl<'a, K: 'a + Clone + Hash + Eq, V: 'a + Clone + Hash + Eq> RelIndexRead<'a>
    for CLatIndex<K, V>
{
    type Key = K;
    type Value = &'a V;
...
impl<'a, K: 'a + Clone + Hash + Eq, V: 'a + Clone + Hash + Eq + Sync> CRelIndexRead<'a>
    for CLatIndex<K, V>
{
    type Key = K;
    type Value = &'a V;
...
impl<'a, K: 'a + Clone + Hash + Eq + Send + Sync, V: 'a + Clone + Hash + Eq + Send + Sync>
    RelIndexWrite for CLatIndex<K, V>
{
    type Key = K;
    type Value = V;
...
impl<'a, K: 'a + Clone + Hash + Eq + Send + Sync, V: 'a + Clone + Hash + Eq + Send + Sync>
    RelIndexMerge for CLatIndex<K, V>
{
    fn move_index_contents(from: &mut Self, to: &mut Self) {
        ...
        to.shards_mut()
            .par_iter_mut()
            .zip(from.shards_mut().par_iter_mut())
            .for_each(|(to, from)| {
...
impl<'a, K: 'a + Clone + Hash + Eq, V: 'a + Clone + Hash + Eq> RelIndexReadAll<'a>
    for CLatIndex<K, V>
{
    type Key = &'a K;
    type Value = V;
...
impl<'a, K: 'a + Clone + Hash + Eq + Sync + Send, V: 'a + Clone + Hash + Eq + Sync + Send>
    CRelIndexReadAll<'a> for CLatIndex<K, V>
{
    type Key = &'a K;
    type Value = &'a V;
    ...
    type AllIteratorType = rayon::iter::Map<
        ...
        for<'aa, 'bb> fn(
            (&'aa K, &'bb SetType<V>),
        ) -> (&'aa K, rayon::collections::hash_set::Iter<'bb, V>),
    >;
