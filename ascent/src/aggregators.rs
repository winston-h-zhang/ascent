...
where
    N: Clone + Into<f64>,
{
    let (sum, count) = inp.fold((0.0, 0usize), |(sum, count), tuple| {
        (tuple.0.clone().into() + sum, count + 1)
    });
    let res = if count == 0 {
        None
    } else {
...
pub fn percentile<'a, TItem: 'a, TInputIter>(
    p: f64,
) -> impl Fn(TInputIter) -> std::option::IntoIter<TItem>
where
