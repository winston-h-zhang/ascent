...
impl<'a, K: Eq + std::hash::Hash, V: 'a + Clone> RelIndexRead<'a>
    for HashBrownRelFullIndexType<K, V>
{
...
impl<'a, K: Eq + std::hash::Hash + 'a, V: 'a + Clone> RelIndexReadAll<'a>
    for HashBrownRelFullIndexType<K, V>
{
    type Key = &'a K;
    type Value = &'a V;
    ...
    for LatticeIndexType<K, V>
{
    ...
    type AllIteratorType = std::iter::Map<
        ...
        for<'aa, 'bb> fn(
            (
                &'aa K,
                &'bb std::collections::HashSet<V, BuildHasherDefault<FxHasher>>,
            ),
...
impl<'a, Ind1, Ind2> RelIndexCombined<'a, Ind1, Ind2> {
    #[inline]
    pub fn new(ind1: &'a Ind1, ind2: &'a Ind2) -> Self {
        Self { ind1, ind2 }
    }
}
