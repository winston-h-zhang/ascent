...
impl<K: Clone + Hash + Eq, V> CRelIndex<K, V> {
    ...
    pub fn unwrap_frozen(
        &self,
    ) -> &dashmap::ReadOnlyView<K, VecType<V>, BuildHasherDefault<FxHasher>> {
        match self {
    ...
    pub fn unwrap_mut_unfrozen(
        &mut self,
    ) -> &mut DashMap<K, VecType<V>, BuildHasherDefault<FxHasher>> {
        match self {
            CRelIndex::Unfrozen(dm) => dm,
            CRelIndex::Frozen(_) => panic!("CRelIndex::unwrap_unfrozen(): object is Frozen"),
        }
...
impl<'a, K: 'a + Clone + Hash + Eq, V: 'a> RelIndexRead<'a> for CRelIndex<K, V> {
    ...
    fn len(&self) -> usize {
        ...
        let (count, sum) = shards
            .iter()
            .take(sample_size)
            .fold((0, 0), |(c, s), shard| (c + 1, s + shard.read().len()));
...
impl<'a, K: 'a + Clone + Hash + Eq + Send + Sync, V: 'a + Send + Sync> RelIndexWrite
    for CRelIndex<K, V>
{
    type Key = K;
    type Value = V;
...
impl<'a, K: 'a + Clone + Hash + Eq + Send + Sync, V: 'a + Send + Sync> RelIndexMerge
    for CRelIndex<K, V>
{
    ...
    for CRelIndex<K, V>
{
    ...
    fn c_iter_all(&'a self) -> Self::AllIteratorType {
        CRelIndexReadAllParIter {
            shards: self.unwrap_frozen().shards(),
        }
    }
}
