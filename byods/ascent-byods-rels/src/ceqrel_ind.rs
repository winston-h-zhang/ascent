...
impl<T: Clone + Hash + Eq> ToRelIndex<CEqRelIndCommon<T>> for ToEqRelIndNone<T> {
    ...
    fn to_rel_index<'a>(&'a self, rel: &'a CEqRelIndCommon<T>) -> Self::RelIndex<'a> {
        EqRelIndNone(rel)
    }
    ...
    type AllIteratorType = FlatMap<
        ...
        Map<
            ...
            for<'aa> fn(
                ...
                &'aa (T,),
                Map<HashSetIter<'aa, T>, for<'bb> fn(&'bb T) -> (&'bb T,)>,
            ),
        >,
        for<'aa> fn(
            ...
            for<'cc> fn(
                ...
                &'cc (T,),
                Map<HashSetIter<'cc, T>, for<'dd> fn(&'dd T) -> (&'dd T,)>,
            ),
        >,
    >;
    ...
    fn iter_all(&'a self) -> Self::AllIteratorType {
        let res: Self::AllIteratorType =
            self.0.unwrap_frozen().combined.sets.iter().flat_map(|s| {
                s.iter()
                    .zip(std::iter::repeat(s.iter()))
                    .map(|(x, s)| (ref_to_singleton_tuple_ref(x), s.map(|x| (x,))))
            });
        res
    }
}
...
impl<'a, T: Clone + Hash + Eq + Sync + Send> ParallelIterator
    for EqRelInd0CRelIndexReadAllIter<'a, T>
{
    type Item = (
        &'a (T,),
...
impl<T: Clone + Hash + Eq> Clone for CEqRelIndCommon<T> {
    fn clone(&self) -> Self {
        match self {
            CEqRelIndCommon::Unfrozen(new) => {
                CEqRelIndCommon::Unfrozen(Mutex::new(new.lock().unwrap().clone()))
            }
...
impl<'a, T: Clone + Hash + Eq + Sync> ParallelIterator for AllAddedParIter<'a, T> {
    ...
    fn drive_unindexed<C>(self, consumer: C) -> C::Result
    where
        C: ascent::rayon::iter::plumbing::UnindexedConsumer<Self::Item>,
    {
        self.0
            .combined
            .c_iter_all()
            .filter(|(x, y)| !self.0.old.contains(x, y))
            .drive_unindexed(consumer)
    }
}
...
impl<'a, T: Clone + Hash + Eq + Sync> ParallelIterator for SetOfAddedParIter<'a, T> {
        ...
        C: ascent::rayon::iter::plumbing::UnindexedConsumer<Self::Item>,
    {
        self.set
            .par_iter()
            .filter(move |y| !self.old_set.map_or(false, |os| os.contains(*y)))
            .drive_unindexed(consumer)
    }
}
...
impl<T: Clone + Hash + Eq> CEqRelIndCommon<T> {
    pub(crate) fn iter_all_added(&self) -> impl Iterator<Item = (&T, &T)> {
        ...
        eqrel
            .combined
            .iter_all()
            .filter(|(x, y)| !eqrel.old.contains(x, y))
    }
...
impl<'a, T: Clone + Hash + Eq + Sync + 'a> CRelIndexReadAll<'a> for CEqRelIndCommon<T> {
    ...
    fn c_iter_all(&'a self) -> Self::AllIteratorType {
        let res: Self::AllIteratorType = self
            .c_iter_all_added()
            .map(|x| (x, ascent::rayon::iter::once(())));
        res
    }
}
