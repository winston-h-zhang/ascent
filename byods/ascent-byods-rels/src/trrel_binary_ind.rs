...
impl<T: Clone + Hash + Eq> RelIndexMerge for TrRelIndCommon<T> {
    ...
    fn init(new: &mut Self, delta: &mut Self, total: &mut Self) {
        ...
        *new = Self::New {
            rel: Default::default(),
            anti_reflexive: delta.anti_reflexive(),
        };
    }
    ...
    fn merge_delta_to_total_new_to_delta(new: &mut Self, delta: &mut Self, total: &mut Self) {
        ...
        fn join<T: Clone + Hash + Eq>(
            ...
            if rel1.len() < rel2_rev.len() {
                for (x, x_set) in rel1.iter() {
                    assert!(
                        !x_set.is_empty(),
                        "bad join {name}. rel1 has non-empty sets"
                    );
                    if let Some(x_rev_set) = rel2_rev.get(x) {
                        assert!(
                            !x_set.is_empty(),
                            "bad join {name}. rel2_rev has non-empty sets"
                        );
                        for w in x_rev_set {
        ...
        *total = TrRelIndCommon::Old {
            rel: total_rel,
            anti_reflexive,
        };
        *delta = TrRelIndCommon::Old {
            rel: new_delta,
            anti_reflexive,
        };
...
impl<'a, T: Clone + Hash + Eq> RelIndexRead<'a> for TrRelIndNone<'a, T> {
    ...
    fn index_get(&'a self, (): &Self::Key) -> Option<Self::IteratorType> {
        println!(
            "iterating TrRelIndNone. {} tuples",
            self.0.rel().map.values().map(|x| x.len()).sum::<usize>()
        );
        let res = || {
            self.0
                .rel()
    ...
    Rel: ToTrRelIndCommon<T>,
{
    ...
    fn to_rel_index<'a>(&'a self, rel: &'a Rel) -> Self::RelIndex<'a> {
        TrRelIndFull(rel.to_tr_rel_ind())
    }
