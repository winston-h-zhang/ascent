    ...
    impl<T: PartialEq> Elems<T> {
        ...
        pub(super) fn iter(&self) -> impl Iterator<Item = (Id, &Elem<T>)> {
            self.0
                .iter()
                .enumerate()
                .map(|(i, e)| (Id(UfPtr::from_usize(i)), e))
        }
        ...
        pub(super) unsafe fn iter_class_unchecked(
            &self,
            start: Id,
        ) -> impl Iterator<Item = (Id, &Elem<T>)> {
        ...
        #[allow(dead_code)]
        pub(super) fn iter_classes(
            &self,
        ) -> impl Iterator<Item = impl Iterator<Item = (Id, &Elem<T>)>> {
            Classes::new(&self)
        }
