...
use ascent::{ascent, ascent_par, ascent_run};
...
fn test_eq_rel2_in_ascent() {
    ...
    println!(
        "indexed_eq_rel_materialized len: {}",
        prog.indexed_eq_rel_materialized.len()
    );
...
fn test_eq_rel2_in_ascent2() {
    let test_cases = vec![
        (0..2)
            .flat_map(|a| (3..5).flat_map(move |x| (9..11).map(move |y| (a, x, y))))
            .collect_vec(),
    ...
    for (i, seed_rel) in test_cases.into_iter().enumerate() {
        ...
        println!(
            "indexed_eq_rel_materialized len: {}",
            res.indexed_eq_rel_materialized.len()
        );
        ...
        assert_eq!(
            res.indexed_eq_rel_materialized.len(),
            res.indexed_eq_rel_explicit.len()
        );
...
fn test_trrel_uf_ternary_in_ascent() {
    ...
    assert_eq!(
        res.__tr_ind_common.0.map[&1].count_exact() as u32,
        (ub + 1).pow(2)
    );
}
...
fn test_trrel2_in_ascent() {
    let test_cases = vec![
        (1..4)
            .flat_map(|a| (3..7).flat_map(move |x| (11..15).map(move |y| (a, x, y))))
            .collect_vec(),
        (0..3)
            .flat_map(|a| {
                (0..15)
            ...
            .collect_vec(),
        (1..4)
            .flat_map(|a| (5..10).flat_map(move |x| (12..17).map(move |y| (a, x, y))))
            .collect_vec(),
        vec![
            (0, 10, 11),
            (0, 5, 13),
    ...
    for (i, seed_rel) in test_cases.into_iter().enumerate() {
        ...
        assert_eq!(
            res.tr_indexed_materialized.len(),
            res.tr_indexed_explicit.len()
        );
    }
}
