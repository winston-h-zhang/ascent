...
use crate::eqrel_ind::{ref_to_singleton_tuple_ref, EqRelIndCommon};
...
{
    fn default() -> Self {
        let reverse_map = if WITH_REVERSE {
            Some(Default::default())
        } else {
            None
        };
        Self(EqRel2IndCommon {
            map: Default::default(),
            reverse_map,
        })
    }
...
impl<const WITH_REVERSE: bool, T0, T1> ToEqRel2IndCommon<T0, T1>
    for EqRel2IndWrapper<WITH_REVERSE, T0, T1>
where
    T0: Clone + Hash + Eq,
    T1: Clone + Hash + Eq,
...
impl<T0: Clone + Hash + Eq, T1: Clone + Hash + Eq> RelFullIndexWrite for EqRel2IndCommon<T0, T1> {
    ...
    fn insert_if_not_present(&mut self, key: &Self::Key, (): Self::Value) -> bool {
        ...
        match self.map.entry(key.0.clone()) {
            hashbrown::hash_map::Entry::Occupied(mut occ) => occ
                .get_mut()
                .insert_if_not_present(&(key.1.clone(), key.2.clone()), ()),
...
impl<T0: Clone + Hash + Eq, T1: Clone + Hash + Eq> RelIndexMerge for EqRel2IndCommon<T0, T1> {
    ...
    fn merge_delta_to_total_new_to_delta(new: &mut Self, delta: &mut Self, total: &mut Self) {
        for (t0, mut delta_eqrel) in delta.map.drain() {
            ...
            RelIndexMerge::merge_delta_to_total_new_to_delta(
                &mut new_eqrel,
                &mut delta_eqrel,
                total_eqrel,
            );
...
impl<T0: Clone + Hash + Eq, T1: Clone + Hash + Eq> EqRel2IndCommon<T0, T1> {
    pub(crate) fn iter_all_added(&self) -> impl Iterator<Item = (&T0, &T1, &T1)> {
        self.map
            .iter()
            .flat_map(|(t0, eqrel)| eqrel.iter_all_added().map(move |(t1, t2)| (t0, t1, t2)))
    }
}
    ...
    for EqRel2Ind0_1<'a, T0, T1>
{
    ...
    fn iter_all(&'a self) -> Self::AllIteratorType {
        Box::new(self.0.map.iter().flat_map(|(t0, eqrel)| {
            eqrel
                .iter_all_added()
                .map(move |(t1, t2)| ((t0, t1), std::iter::once((t2,))))
        }))
    }
}
    ...
    for EqRel2Ind0<'a, T0, T1>
{
    ...
    fn iter_all(&'a self) -> Self::AllIteratorType {
        Box::new(self.0.map.iter().map(|(t, eqrel)| {
            (
                ref_to_singleton_tuple_ref(t),
                IteratorFromDyn::new(|| eqrel.iter_all_added()),
            )
        }))
    }
}
...
impl<'a, T0: Clone + Hash + Eq, T1: Clone + Hash + Eq> RelIndexReadAll<'a>
    for EqRel2Ind1_2<'a, T0, T1>
{
    type Key = (&'a T1, &'a T1);
    type Value = (&'a T0,);

    type ValueIteratorType = Map<
        hashbrown::hash_set::Intersection<'a, T0, BuildHasherDefault<FxHasher>>,
        for<'aa> fn(&'aa T0) -> (&T0,),
    >;
    ...
    fn iter_all(&'a self) -> Self::AllIteratorType {
        let res = self
            ...
            .map(|((t0, t0_set), (t1, t1_set))| {
                let intersection: Self::ValueIteratorType =
                    t0_set.intersection(t1_set).map(|x| (x,));
                ((t0, t1), intersection)
            });
        Box::new(res)
    }
    ...
    for EqRel2IndFull<'a, T0, T1>
{
    ...
    fn contains_key(&'a self, (t0, t1, t2): &Self::Key) -> bool {
        if let Some(eqrel) = self.0.map.get(t0) {
            eqrel.added_contains(t1, t2)
        } else {
            false
        }
    }
