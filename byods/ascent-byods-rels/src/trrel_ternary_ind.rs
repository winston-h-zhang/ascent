...
{
    fn default() -> Self {
        let reverse_map1 = if HAS_REVERSE_MAP1 {
            Some(Default::default())
        } else {
            None
        };
        let reverse_map2 = if HAS_REVERSE_MAP2 {
            Some(Default::default())
        } else {
            None
        };
        Self(TrRel2IndCommon {
            map: Default::default(),
            reverse_map1,
            reverse_map2,
        })
...
pub struct TrRel2IndCommon<T0: Clone + Hash + Eq, T1: Clone + Hash + Eq> {
    ...
    pub reverse_map1: Option<
        HashMap<T1, RevMapHashSet<T0, BuildHasherDefault<FxHasher>>, BuildHasherDefault<FxHasher>>,
    >,
    pub reverse_map2: Option<
        HashMap<T1, RevMapHashSet<T0, BuildHasherDefault<FxHasher>>, BuildHasherDefault<FxHasher>>,
    >,
}
...
impl<T0: Clone + Hash + Eq, T1: Clone + Hash + Eq> RelIndexMerge for TrRel2IndCommon<T0, T1> {
    ...
    fn merge_delta_to_total_new_to_delta(new: &mut Self, delta: &mut Self, total: &mut Self) {
        ...
        for (k, mut delta_trrel) in delta.map.drain() {
            let mut new_trrel = new
                .map
                .remove(&k)
                .unwrap_or_else(|| TrRelIndCommon::make_new());
            match total.map.entry(k.clone()) {
                ...
                hashbrown::hash_map::Entry::Vacant(total_vacant_entry) => {
                    let mut new_total = TrRelIndCommon::Old {
                        rel: Default::default(),
                        anti_reflexive: delta_trrel.anti_reflexive(),
                    };
                    TrRelIndCommon::merge_delta_to_total_new_to_delta(
                        &mut new_trrel,
                        &mut delta_trrel,
        ...
        if delta.reverse_map1.is_some() {
            ...
            std::mem::swap(
                delta.reverse_map1.as_mut().unwrap(),
                new.reverse_map1.as_mut().unwrap(),
            );
        }
        ...
        if delta.reverse_map2.is_some() {
            ...
            std::mem::swap(
                delta.reverse_map2.as_mut().unwrap(),
                new.reverse_map2.as_mut().unwrap(),
            );
        }
    }
}
...
impl<'a, T0: Clone + Hash + Eq, T1: Clone + Hash + Eq> RelIndexReadAll<'a>
    for TrRel2Ind0<'a, T0, T1>
{
    type Key = (&'a T0,);
    ...
    fn iter_all(&'a self) -> Self::AllIteratorType {
        Box::new(
            self.0
                .map
                .iter()
                .map(|(k, v)| ((k,), Box::new(v.rel().iter_all()) as _)),
        )
    }
}
...
impl<'a, T0: Clone + Hash + Eq, T1: Clone + Hash + Eq> RelIndexRead<'a>
    for TrRel2Ind0_1<'a, T0, T1>
{
    type Key = (T0, T1);
    ...
    fn iter_all(&'a self) -> Self::AllIteratorType {
        Box::new(
            self.0
                ...
                .unwrap()
                .iter()
                .flat_map(move |(x1, x0s_for_x1)| {
                    self.0
                        .reverse_map2
                        .as_ref()
                        .unwrap()
                        .iter()
                        .map(move |(x2, x0s_for_x2)| {
                            let x0s: Self::ValueIteratorType = Box::new(
                                x0s_for_x1
                                    .intersection(x0s_for_x2)
                                    .filter(|&x0| {
                                        self.0.map.get(x0).unwrap().rel().contains(x1, x2)
                                    })
                                    .map(|x0| (x0,)),
                            );
                            ((x1, x2), x0s)
    ...
    fn index_get(&'a self, (): &Self::Key) -> Option<Self::IteratorType> {
        let res = || {
            self.0
                .map
                .iter()
                .flat_map(|(x0, trrel)| trrel.rel().iter_all().map(move |(x1, x2)| (x0, x1, x2)))
        };
        Some(IteratorFromDyn::new(res))
    }
...
impl<'a, T0: Clone + Hash + Eq, T1: Clone + Hash + Eq> RelFullIndexRead<'a>
    for TrRel2IndFull<'a, T0, T1>
{
    type Key = (T0, T1, T1);
    ...
    for TrRel2IndFull<'a, T0, T1>
{
    ...
    fn len(&self) -> usize {
        ...
        let sum = self
            ...
            .values()
            .take(sample_size)
            .map(|trrel| trrel.rel().count_estimate())
            .sum::<usize>();
        let map_len = self.0.map.len();
        sum * map_len / sample_size.min(map_len).max(1)
    }
...
impl<'a, T0: Clone + Hash + Eq, T1: Clone + Hash + Eq> RelIndexMerge
    for TrRel2IndFullWrite<'a, T0, T1>
{
    fn move_index_contents(_from: &mut Self, _to: &mut Self) {} // noop
}

impl<'a, T0: Clone + Hash + Eq, T1: Clone + Hash + Eq> RelFullIndexWrite
    for TrRel2IndFullWrite<'a, T0, T1>
{
    type Key = (T0, T1, T1);
    ...
    fn insert_if_not_present(&mut self, (x0, x1, x2): &Self::Key, (): Self::Value) -> bool {
        ...
        if let Some(reverse_map1) = self.0.reverse_map1.as_mut() {
            reverse_map1
                .entry(x1.clone())
                .or_default()
                .insert_with_hash_no_check(x0_hash, x0.clone());
...
impl<'a, T0: Clone + Hash + Eq, T1: Clone + Hash + Eq> RelIndexWrite
    for TrRel2IndFullWrite<'a, T0, T1>
{
    type Key = (T0, T1, T1);
    ...
    fn index_insert(&mut self, (x0, x1, x2): Self::Key, (): Self::Value) {
        ...
        self.0
            .map
            .entry(x0)
            .or_insert_with(|| TrRelIndCommon::make_new())
            .insert(x1, x2);
    }
}
