...
impl<'a, T: Clone + Hash + Eq + 'a> RelIndexReadAll<'a> for MapRelIndexAdaptor<'a, T> {
    ...
    type AllIteratorType = std::iter::Map<
        ...
        for<'aa> fn(
            (&'aa T, &'a MyHashSet<T, BuildHasherDefault<FxHasher>>),
        ) -> (&'aa T, Self::ValueIteratorType),
    >;
    ...
    F: Fn(V) -> U,
{
    ...
    fn index_get(&'a self, key: &Self::Key) -> Option<Self::IteratorType> {
        let res: Self::IteratorType = self
            .rel
            .index_get(key)?
            .zip(std::iter::repeat(self))
            .map(|(x, _self)| (_self.f)(x));
        Some(res)
    }
    ...
    F: Fn(V) -> U,
{
    ...
    fn iter_all(&'a self) -> Self::AllIteratorType {
        let res = self.rel.iter_all().map(move |(k, vals_iter)| {
            let new_vals_iter: Self::ValueIteratorType = vals_iter
                .zip(std::iter::repeat(self))
                .map(|(x, _self)| (_self.f)(x));
            (k, new_vals_iter)
        });
