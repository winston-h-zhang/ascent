    ...
    TBinRel: ByodsBinRel<T0 = T1, T1 = T2>,
{
    ...
    pub reverse_map1: Option<
        HashMap<T1, AltHashSet<T0, BuildHasherDefault<FxHasher>>, BuildHasherDefault<FxHasher>>,
    >,
    pub reverse_map2: Option<
        HashMap<T2, AltHashSet<T0, BuildHasherDefault<FxHasher>>, BuildHasherDefault<FxHasher>>,
    >,
}
        ...
        for (k, mut delta_trrel) in delta.map.drain() {
            ...
            match total.map.entry(k.clone()) {
                hashbrown::hash_map::Entry::Occupied(mut total_entry) => {
                    TBinRel::merge_delta_to_total_new_to_delta(
                        &mut new_trrel,
                        &mut delta_trrel,
                        total_entry.get_mut(),
                    );
    ...
    fn iter_all(&'a self) -> Self::AllIteratorType {
        Box::new(
            self.0
                .map
                .iter()
                .map(|(k, v)| ((k,), Box::new(v.iter_all()) as _)),
        )
    }
}
    ...
    TBinRel: ByodsBinRel<T0 = T1, T1 = T2>,
{
    ...
    fn len(&self) -> usize {
        ...
        let sum = self
            ...
            .values()
            .take(sample_size)
            .map(|trrel| trrel.ind0_len_estimate())
            .sum::<usize>();
    ...
    TBinRel: ByodsBinRel<T0 = T1, T1 = T2>,
{
    ...
    fn len(&self) -> usize {
        ...
        let sum = self
            ...
            .values()
            .take(sample_size)
            .map(|trrel| trrel.ind1_len_estimate())
            .sum::<usize>();
        let map_len = self.0.map.len();
        sum * map_len / sample_size.min(map_len).max(1)
    }
...
pub struct BinRelToTernaryIndNone<'a, T0, T1, T2, TBinRel>(
    &'a BinRelToTernary<T0, T1, T2, TBinRel>,
)
where
    T0: Clone + Hash + Eq,
    T1: Clone + Hash + Eq,
...
impl<'a, T0, T1, T2, TBinRel> RelIndexReadAll<'a>
    for BinRelToTernaryIndNone<'a, T0, T1, T2, TBinRel>
where
    T0: Clone + Hash + Eq,
    T1: Clone + Hash + Eq,
    T2: Clone + Hash + Eq,
    ...
    fn index_get(&'a self, (): &Self::Key) -> Option<Self::IteratorType> {
        let res = || {
            self.0
                .map
                .iter()
                .flat_map(|(x0, rel)| rel.iter_all().map(move |(x1, x2)| (x0, x1, x2)))
        };
        Some(IteratorFromDyn::new(res))
    }
...
pub struct BinRelToTernaryInd0_1_2<'a, T0, T1, T2, TBinRel>(
    &'a BinRelToTernary<T0, T1, T2, TBinRel>,
)
where
    T0: Clone + Hash + Eq,
...
impl<'a, T0, T1, T2, TBinRel> RelFullIndexRead<'a>
    for BinRelToTernaryInd0_1_2<'a, T0, T1, T2, TBinRel>
where
    T0: Clone + Hash + Eq,
    T1: Clone + Hash + Eq,
    T2: Clone + Hash + Eq,
...
pub struct BinRelToTernaryInd0_1_2Write<'a, T0, T1, T2, TBinRel>(
    &'a mut BinRelToTernary<T0, T1, T2, TBinRel>,
)
where
    T0: Clone + Hash + Eq,
    T1: Clone + Hash + Eq,
...
impl<'a, T0, T1, T2, TBinRel> RelIndexMerge
    for BinRelToTernaryInd0_1_2Write<'a, T0, T1, T2, TBinRel>
where
    T0: Clone + Hash + Eq,
    T1: Clone + Hash + Eq,
    T2: Clone + Hash + Eq,
...
impl<'a, T0, T1, T2, TBinRel> RelFullIndexWrite
    for BinRelToTernaryInd0_1_2Write<'a, T0, T1, T2, TBinRel>
where
    T0: Clone + Hash + Eq,
    T1: Clone + Hash + Eq,
...
impl<'a, T0, T1, T2, TBinRel> RelIndexWrite
    for BinRelToTernaryInd0_1_2Write<'a, T0, T1, T2, TBinRel>
where
    T0: Clone + Hash + Eq,
    T1: Clone + Hash + Eq,
    ...
    TBinRel: ByodsBinRel<T0 = T1, T1 = T2>,
{
    ...
    fn index_insert(&mut self, (x0, x1, x2): Self::Key, (): Self::Value) {
        ...
        self.0
            .map
            .entry(x0)
            .or_insert_with(|| TBinRel::default())
            .insert(x1, x2);
    }
}

pub struct BinRelToTernaryWrapper<
    ...
    // TWrapper: DerefMut<Target = BinRelToTernary<T0, T1, T2, TBinRel>>
{
    ...
    fn to_rel_index<'a>(
        &'a self,
        rel: &'a BinRelToTernaryWrapper<HAS_REVERSE_MAP1, HAS_REVERSE_MAP2, T0, T1, T2, TBinRel>,
    ) -> Self::RelIndex<'a> {
    ...
    fn to_rel_index_write<'a>(
        &'a mut self,
        rel: &'a mut BinRelToTernaryWrapper<
            HAS_REVERSE_MAP1,
            HAS_REVERSE_MAP2,
            T0,
            T1,
