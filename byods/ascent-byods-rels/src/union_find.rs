...
impl<'a, T: Clone + Hash + Eq + Sync> ParallelIterator for IterAllParIterator<'a, T> {
        ...
        C: ascent::rayon::iter::plumbing::UnindexedConsumer<Self::Item>,
    {
        self.0
            ...
            .flat_map::<fn(_) -> _, _>(|s| {
                s.par_iter()
                    .map_with(s, |s, x| s.par_iter().map_with(x, |x, y| (*x, y)))
                    .flatten()
            })
...
impl<T: Clone + Hash + Eq> EqRel<T> {
    ...
    pub fn c_set_of(
        &self,
        x: &T,
    ) -> Option<&'_ hashbrown::hash_set::HashSet<T, BuildHasherDefault<FxHasher>>>
    where
        T: Sync,
    {
        let set = self.elem_set(x)?;
    ...
    pub fn iter_all(&self) -> IterAllIterator<'_, T> {
        let res: IterAllIterator<'_, T> = self.sets.iter().flat_map(|s| {
            s.iter()
                .zip(std::iter::repeat(s.iter()))
                .flat_map(|(x, s)| s.zip(std::iter::repeat(x)))
        });
        res
    }
    ...
    pub fn contains(&self, x: &T, y: &T) -> bool {
        self.elem_set(x)
            .map_or(false, |set| self.sets[set].contains(y))
    }
