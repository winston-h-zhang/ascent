...
impl<T: Clone + Hash + Eq> TrRelDelta<T> {
    ...
    fn ind_0_iter_all<'a>(&'a self) -> IteratorFromDyn<(&'a T, IteratorFromDyn<&'_ T>)> {
        let res = || {
            self.set_connections
                .iter()
                .flat_map(|(set_id, set_connections)| {
                    let xs = &self.total.sets[*set_id];
                    xs.iter().map(|x| {
                        let ys = || {
                            set_connections
                                .iter()
                                .flat_map(|sc| self.total.sets[*sc].iter())
                        };
    ...
    fn ind_1_iter_all<'a>(&'a self) -> IteratorFromDyn<(&'a T, IteratorFromDyn<&'_ T>)> {
        let res = || {
            self.rev_set_connections
                .iter()
                .flat_map(|(set_id, rev_set_connections)| {
                    let xs = &self.total.sets[*set_id];
                    xs.iter().map(|x| {
                        let ys = || {
                            rev_set_connections
    ...
    pub fn unwrap_new_mut(&mut self) -> &mut NewSet<T> {
        match self {
            ...
            _ => {
                assert!(
                    self.is_empty(),
                    "unwrap_new_mut called on non-empty non-New"
                );
                *self = TrRelIndCommon::New {
                    rel: Default::default(),
                };
                self.unwrap_new_mut()
            }
...
impl<T: Clone + Hash + Eq> RelIndexMerge for TrRelIndCommon<T> {
    ...
    fn merge_delta_to_total_new_to_delta(new: &mut Self, delta: &mut Self, total: &mut Self) {
        ...
        if total_rel_rc.is_empty() && delta_rel.is_empty() {
            ...
            *delta = TrRelIndCommon::Total {
                rel: Rc::new(new_delta),
            };
            return;
        }
        ...
        type RelMap<T> =
            HashMap<T, MyHashSet<T, BuildHasherDefault<FxHasher>>, BuildHasherDefault<FxHasher>>;
        ...
        fn join<
            'a,
            T: Clone + Hash + Eq + 'a,
            Rel1: RelIndexRead<'a, Key = T, Value = &'a T>
                + RelIndexReadAll<'a, Key = &'a T, Value = &'a T>,
            Rel2Rev: RelIndexRead<'a, Key = T, Value = &'a T>
                + RelIndexReadAll<'a, Key = &'a T, Value = &'a T>,
        >(
            target: &mut RelMap<T>,
            target_rev: &mut RelMap<T>,
            rel1: &'a Rel1,
            rel2_rev: &'a Rel2Rev,
        ...
        loop {
            ...
            move_hash_map_of_hash_set_contents_disjoint(
                &mut delta_delta_rev_map,
                &mut delta_total_rev_map,
            );
    ...
    fn len_estimate(&self) -> usize {
        ...
        match self {
            TrRelIndCommon::Delta { rel, .. } => {
                let avg_set_connections = rel
                    .set_connections
                    .iter()
                    .take(3)
                    .map(|(_s, sets)| sets.len())
                    .sum::<usize>()
                    / sample_size.min(rel.set_connections.len()).max(1);
                let avg_set_size = rel
                    .total
                    .sets
                    .iter()
                    .rev()
        ...
        let res = match self {
            TrRelIndCommon::Delta { rel, .. } => {
                ...
                let sum_set_size = rel
                    ...
                    .map(|s| s.len())
                    .sum::<usize>();
                sum_set_size * rel.set_connections.len()
                    / sample_size.min(rel.total.sets.len()).max(1)
            }
            TrRelIndCommon::Total { rel, .. } => {
                let sample_size = 3;
        ...
        let res = match self {
            TrRelIndCommon::Delta { rel, .. } => {
                ...
                let sum_set_size = rel
                    ...
                    .map(|s| s.len())
                    .sum::<usize>();
                sum_set_size * rel.rev_set_connections.len()
                    / sample_size.min(rel.total.sets.len()).max(1)
            }
            TrRelIndCommon::Total { rel, .. } => {
                let sample_size = 3;
                let sum_set_size = rel
                    ...
                    .map(|s| s.len())
                    .sum::<usize>();
                sum_set_size * rel.reverse_set_connections.len()
                    / sample_size.min(rel.sets.len()).max(1)
            }
            TrRelIndCommon::New { .. } => panic!("unexpected New"),
        };
