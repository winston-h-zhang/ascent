...
impl<T: Clone + Hash + Eq> TrRelUnionFind<T> {
    ...
    pub fn add(&mut self, x: T, y: T) -> bool {
        ...
        if self
            .set_connections
            .get(&y_set)
            .map_or(false, |y_set| y_set.contains(&x_set))
        {
            ...
            keep_difference(
                self.reverse_set_connections.get_mut(&x_set).unwrap(),
                &to_be_merged,
            );
    ...
    fn add_set_connection(&mut self, from: usize, to: usize) -> bool {
        ...
        let from_reverse_connections =
            std::mem::take(self.reverse_set_connections.entry(from).or_default());
        ...
        let new_to_connections: HashSet<usize, BuildHasherDefault<FxHasher>> = to_connections
            .difference(&self.set_connections[&from])
            .cloned()
            .collect();
        let new_from_reverse_connections: HashSet<usize, BuildHasherDefault<FxHasher>> =
            from_reverse_connections
                .difference(&self.reverse_set_connections[&to])
                .cloned()
                .collect();
        ...
        self.reverse_set_connections
            .entry(to)
            .or_default()
            .extend(from_reverse_connections.iter().cloned());
        self.set_connections
            .entry(from)
            .or_default()
            .extend(to_connections.iter().cloned());

        self.reverse_set_connections
            .insert(from, from_reverse_connections);
    ...
    pub(crate) fn rev_set_of_by_set_id<'a>(
        &'a self,
        _x: &T,
        id: usize,
    ...
    pub fn iter_all(&self) -> impl Iterator<Item = (&'_ T, &'_ T)> {
        self.elem_ids
            .iter()
            .flat_map(|(x, &x_set_id)| self.set_of_by_set_id(x, x_set_id).map(move |y| (x, y)))
    }
    ...
    pub fn contains(&self, x: &T, y: &T) -> bool {
        self.elem_set(x).map_or(false, |set| {
            self.sets[set].contains(y) || {
                // TODO
                self.get_set_connections(set)
                    .into_iter()
                    .flatten()
                    .any(|s| self.sets[s].contains(y))
            }
        })
    }
    ...
    pub fn get_reverse_set_connections(
        &self,
        set: usize,
    ...
    pub fn count_exact(&self) -> usize {
        let dominant_sets: HashSet<usize> = (0..self.sets.len())
            .map(|s| self.get_dominant_id(s))
            .collect();
        let mut res = 0;
    ...
    pub fn assert_set_connections_dominant_sets(&self) {
        let dominated_sets: HashSet<usize, BuildHasherDefault<FxHasher>> = (0..self.sets.len())
            .filter(|s| self.set_subsumptions.get(s).is_some())
            .collect();
        ...
        for (i, rsc) in self.reverse_set_connections.iter() {
            ...
            assert!(
                intersect.is_empty(),
                "set {} has dominated reverse connections: {:?}",
                i,
                intersect
            );
        }
...
fn keep_difference<T: Hash + Eq, S: BuildHasher>(
    set: &mut HashSet<T, S>,
    to_subtract: &HashSet<T, S>,
) {
...
#[allow(dead_code)]
fn extend_set<T: Hash + Eq + Clone, S: BuildHasher + Clone>(
    set1: &mut HashSet<T, S>,
    set2: &HashSet<T, S>,
) {
...
mod tests {
    ...
    fn test_trrel_union_find3() {
        ...
        println!(
            "set of 10000: {:?}",
            rel.set_of(&10000).unwrap().collect_vec().iter().sorted()
        );
    ...
    fn test_trrel_union_find_ground_truth() {
        ...
        for i in 0..tests_count {
            ...
            println!(
                "test {i}: tuples_count: {tuples_count}, ground_truth_res tuples: {}",
                ground_truth_prog.rtc.len()
            );
