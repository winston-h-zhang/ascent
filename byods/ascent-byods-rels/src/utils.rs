...
pub(crate) fn move_hash_map_of_hash_set_contents<K, V, S1, S2>(
    from: &mut HashMap<K, HashSet<V, S2>, S1>,
    to: &mut HashMap<K, HashSet<V, S2>, S1>,
) where
    K: Clone + Hash + Eq,
    V: Clone + Hash + Eq,
    S1: BuildHasher,
...
pub(crate) fn move_hash_map_of_vec_contents<K, V, S1>(
    from: &mut HashMap<K, Vec<V>, S1>,
    to: &mut HashMap<K, Vec<V>, S1>,
) where
    K: Clone + Hash + Eq,
    V: Clone + Hash + Eq,
    S1: BuildHasher,
...
pub fn move_hash_set_contents_disjoint<T: Hash + Eq, S: BuildHasher>(
    from: &mut HashSet<T, S>,
    to: &mut HashSet<T, S>,
) {
    if from.len() > to.len() {
        std::mem::swap(from, to);
...
pub(crate) fn move_hash_map_of_alt_hash_set_contents<K, V, S1, S2>(
    from: &mut HashMap<K, AltHashSet<V, S2>, S1>,
    to: &mut HashMap<K, AltHashSet<V, S2>, S1>,
) where
    K: Clone + Hash + Eq,
    V: Clone + Hash + Eq,
    S1: BuildHasher,
...
impl<T: Clone + Hash + Eq, S: BuildHasher> AltHashSet<T, S> {
    ...
    pub fn insert_with_hash_no_check(&mut self, hash: u64, item: T) {
        self.0
            .raw_entry_mut()
            .from_key_hashed_nocheck(hash, &item)
            .or_insert(item, ());
    }
...
{
    if hm.len() < hs.len() {
        IteratorFromDyn::new(move || {
            hm.iter()
                .filter_map(move |(k, v)| if hs.contains(k) { Some(v) } else { None })
        })
    } else {
...
pub(crate) fn merge_sets<T: Hash + Eq, S: BuildHasher>(
    set1: &mut HashSet<T, S>,
    mut set2: HashSet<T, S>,
) {
