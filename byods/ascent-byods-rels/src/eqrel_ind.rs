...
fn test_eq_rel_ind_0_iter_all() {
    ...
    let eq_rel_full_ind = EqRelIndCommon {
        old: Rc::new(eq_rel_old),
        combined: Rc::new(eq_rel_new),
    };
    let eq_rel_ind_0 = EqRelInd0(&eq_rel_full_ind);
    let iter = eq_rel_ind_0
        .iter_all()
        .map(|x| (x.0, x.1.collect_vec()))
        .collect_vec();
    ...
    for x in [1, 9, 103] {
        ...
        assert_eq!(
            iter_at_x.into_iter().map(|x| *x.0).collect::<HashSet<_>>(),
            (1..=10).chain(101..=110).collect()
        );
    }
}
...
impl<T: Clone + Hash + Eq> EqRelIndCommon<T> {
    pub fn iter_all_added(&self) -> impl Iterator<Item = (&T, &T)> {
        self.combined
            .iter_all()
            .filter(|(x, y)| !self.old.contains(x, y))
    }
...
impl<'a, T: Clone + Hash + Eq> RelIndexMerge for EqRelIndCommon<T> {
    ...
    fn merge_delta_to_total_new_to_delta(new: &mut Self, delta: &mut Self, total: &mut Self) {
        ...
        Rc::make_mut(&mut delta.combined)
            .combine(std::mem::take(Rc::get_mut(&mut new.combined).unwrap()));
    }
}
